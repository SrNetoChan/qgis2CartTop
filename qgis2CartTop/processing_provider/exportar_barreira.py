...
class ExportarBarreira(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        input_layer = self.addParameter(
            QgsProcessingParameterFeatureSource(
                self.INPUT,
                self.tr(' Camada de linha ou polígono de entrada'),
                types=[QgsProcessing.TypeVectorLine, QgsProcessing.TypeVectorPolygon],
                defaultValue=None
            )
        )

        self.vb_keys, self.vb_values = get_lista_codigos('valorBarreira')
        self.addParameter(
            QgsProcessingParameterEnum(
                self.VALOR_BARREIRA,
                self.tr('Valor Barreira'),
                self.vb_keys,
                defaultValue=0,
                optional=False,
            )
    ...
    def shortHelpString(self):
        return self.tr("Exporta elementos do tipo Barreira para a base " \
                       "de dados RECART usando uma ligação PostgreSQL/PostGIS " \
                       "já configurada.\n\n" \
                       "A camada vectorial de input deve ser do tipo linha ou polígono 2D."
        )